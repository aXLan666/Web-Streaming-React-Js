import ProdukFilm from "../../models/ProdukModel/FilmProduk.js";
import Film from "../../models/VideoModel/FilmModal.js";

export const getProdukFilms = async (req, res) => {
  try {
    const response = await ProdukFilm.findAll();
    res.json(response);
  } catch (error) {
    console.log(error.message);
    res.status(500).json({ error: "Failed to fetch produk films" });
  }
};

export const getProdukFilmById = async (req, res) => {
  try {
    const response = await ProdukFilm.findOne({
      where: {
        id: req.params.id
      }
    });
...
export const deleteFilm = async (req, res) => {
  try {
    ...
    const filmToDelete = await ProdukFilm.findOne({ where: { id } });
