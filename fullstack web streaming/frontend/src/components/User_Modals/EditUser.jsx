...
function EditUser({ isOpen, closeModal, user }) {
  ...
  const updateUser = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`http://localhost:5009/users/${user.uuid}`,
        {
          name: name,
          email: email,
          password: password,
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray700">
          <div className="flex items-start justify-between p-4 border-b rounded-t dark:border-gray600">
            ...
            <button
              onClick={closeModal}
              type="button"
              className="text-gray400 bg-transparent hover:bg-gray200 hover:text-gray900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray600 dark:hover:text-white"
            >
              <svg
                aria-hidden="true"
                className="w-5 h-5"
            ...
            <form onSubmit={updateUser}>
              ...
              <div className="field mb-2">
                <div className="control">
                  <select
                    className="select select-bordered w-full max-w-mix text-lg"
                    value={role}
